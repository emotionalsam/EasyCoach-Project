...
export default function Upload({ setData }) {
  ...
  const getInfo = (data, titles, fun) => {
    ...
    return [
      ...
      fun === "SUM"
        ? average.slice(2)
        : average.slice(2).map((e) => e / data.length),
    ];
  };
  ...
  return (
    <CSVReader
      onUploadAccepted={({ data }) => {
        ...
        let types = {};
        let [average, averageArr] = getInfo(tableRows, tableTitle, "AVG");
        let [sum, sumArr] = getInfo(tableRows, tableTitle, "SUM");
        console.log(averageArr, sumArr);
